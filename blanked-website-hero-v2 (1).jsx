...
const AchievementsScreen = () => {
  const tc = { bronze: "#CD7F32", silver: "#9CA3A8", gold: "#D4A012", none: "#E8E6E1" };
  const badges = [
            ...
            <div key={i} style={{ padding: "7px 8px", borderRadius: 10, background: "white", border: b.t!=="none" ? `1.5px solid ${col}35` : "1.5px solid rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", gap: 3 }}>
                  {b.dots.map((f,j) => (<div key={j} style={{ width: 6, height: 6, borderRadius: 3, background: f ? tc[["bronze","silver","gold"][j]] : "#E8E6E1" }} />))}
                </div>
...
export default function HeroSection() {
  ...
  useEffect(() => { const t = setInterval(() => setActiveIdx(i => (i+1)%5), 5000); return () => clearInterval(t); }, []);
    ...
    <div style={{
      ...
      <div style={{ flex: 1, maxWidth: 480 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 48 }}>
          <div style={{ width: 40, height: 40, borderRadius: 12, background: `linear-gradient(135deg, ${P.accent}, ${P.accentL})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="22" height="14" viewBox="0 0 36 24"><path d="M2 12Q18 2 34 12Q18 22 2 12Z" fill="rgba(255,255,255,0.25)" stroke="white" strokeWidth="1.2"/><circle cx="18" cy="12" r="4" fill="white"/></svg>
          </div>
        ...
        <p style={{ fontSize: 17, color: P.textM, lineHeight: 1.6, margin: "0 0 32px" }}>
          ...
          A visual memory game that starts simple and gets impossibly hard.
        </p>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 40 }}>
          <div style={{ padding: "14px 28px", borderRadius: 14, background: P.text, color: "white", display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 15, fontWeight: 600 }}>
        ...
        <div style={{ display: "flex", gap: 40 }}>
          {[{v:"380+",l:"Levels"},{v:"6",l:"Game modes"},{v:"30s",l:"Per round"}].map((s,i) => (
            <div key={i}><div style={{ fontSize: 28, fontWeight: 800, color: P.text }}>{s.v}</div><div style={{ fontSize: 13, color: P.textD, marginTop: 2 }}>{s.l}</div></div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        <div style={{ animation: "phoneFloat 6s infinite ease-in-out" }}>
          <PhoneFrame>
            <div key={activeIdx} style={{ height: "100%", animation: "heroFadeIn 0.6s ease-out" }}><ActiveScreen /></div>
          </PhoneFrame>
        </div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {modeLabels.map((m,i) => (
            <div key={i} onClick={() => setActiveIdx(i)} style={{ width: i===activeIdx?20:8, height: 8, borderRadius: 4, background: i===activeIdx?m.color:"#E8E6E1", cursor: "pointer", transition: "all 0.3s" }} />
          ))}
        </div>
      </div>
    </div>
